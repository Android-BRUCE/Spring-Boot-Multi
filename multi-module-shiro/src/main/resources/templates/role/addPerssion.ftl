...
<script>
    ...
    $('#tree02').tree({
        ...
        //连线
        lines: true,
        url: '${basePath}/permission/getPerssionTree.do?roleId='+$("#id").val(),
        ...
        onCheck: function (node, checked) {
            ...
            if (checked) {
                    $.ajax({
                        type: "POST",
                        url: "${basePath}/role/changeTreeRolePerssion.do?perssionId="+node.id+"&status=1&roleId="+$("#id").val(),
                        cache: false,
                        dataType: "json",
                        success: function (results) {
                            /*for(var i=0;i<results.length;i++){
                                if (myMap.get(results[i].id) === null){
                                    myMap.put(results[i].id, "0");
                                }
                            }*/
            ...
            if(!checked) {
                    $.ajax({
                        type: "POST",
                        url: "${basePath}/role/changeTreeRolePerssion.do?perssionId="+node.id+"&status=0&roleId="+$("#id").val(),
                        cache: false,
                        dataType: "json",
                        success: function (results) {
                            /*for(var i=0;i<results.length;i++){
                                if (myMap.get(results[i].id) !== null){
                                    myMap.remove(results[i].id);
                                }
                            }*/
                        }
                    });
