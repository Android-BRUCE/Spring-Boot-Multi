...
<body class="gray-bg">
	...
	<script>
		function addModel() {
			...
			window.location.href = "${basePath}/permissionInit/editPage/add";
		}
		...
		function editModel() {//编辑
			...
			if (row != null) {
				...
				window.location.href = "${basePath}/permissionInit/editPage/" + row.id;
			} else {
		...
		function delData() {//删除
			XPage.DelData("${basePath}/permissionInit/delete");
		}
		...
		$(document).ready(function() {
			var config = {
				title : '初始权限列表',
				url : '${basePath}/permissionInit/getPermissionInitListWithPager',
				colNames : [ '主键', 'url地址', 'url权限' ,'序号'],
				colModel : [ {
					name : 'id',
					index : 'id',
					width : 60,
